...
class PartnerRepository {
    ...
    static getFullInformation = async (partnerId: number) => {
        const lang = i18n.language.charAt(0).toUpperCase() + i18n.language.slice(1);
        let partner: any = await sequelize.query('SELECT "partner"."id" as "id", "partner"."statusId" as "statusId", "partner"."name'+lang+'" as "name", "partner"."tradeName'+lang+'" as "tradeName", "partner"."license" as "license","partner"."ceoFirstName'+lang+'" as "ceoFirstName", "partner"."ceoLastName'+lang+'" as "ceoLastName", "partner"."establishmentYear" as "establishmentYear", "partner"."employersCount" as "employersCount", "partner"."tel" as "tel", "partner"."website" as "website", "partner"."city'+lang+'" as "city", "partner"."address'+lang+'" as "address", "partner"."zip" as "zip", TO_CHAR("partner"."createdAt", \'yyyy-mm-dd HH:MI:SS\') as "createdAt", "country"."title" AS "country", "areaOfWork"."title' + lang + '" AS "areaOfWork", "ownership"."title' + lang + '" AS "ownership", "partnerType"."title' + lang + '" AS "partnerType", "csoType"."title' + lang + '" AS "csoType", "assist"."assistId" as "assistant.id", "assist"."email" as "assistant.email", "assist"."firstName" as "assistant.firstName", "assist"."lastName" as "assistant.lastName", "assist"."occupation" as "assistant.occupation", "assist"."tel" as "assistant.tel", "assist"."mobile" as "assistant.mobile", "authorised"."authorisedId" as "authorised.id", "authorised"."email" as "authorised.email", "authorised"."firstName" as "authorised.firstName", "authorised"."lastName" as "authorised.lastName", "authorised"."occupation" as "authorised.occupation", "authorised"."tel" as "authorised.tel", "authorised"."mobile" as "authorised.mobile" FROM "partners" AS "partner" LEFT OUTER JOIN "countries" AS "country" ON "partner"."countryId" = "country"."id" LEFT OUTER JOIN "areas_of_work" AS "areaOfWork" ON "partner"."areaOfWorkId" = "areaOfWork"."id" LEFT OUTER JOIN "companys_ownerships" AS "ownership" ON "partner"."ownershipId" = "ownership"."id" LEFT OUTER JOIN "partner_types" AS "partnerType" ON "partner"."partnerTypeId" = "partnerType"."id" LEFT OUTER JOIN "cso_types" AS "csoType" ON "partner"."csoTypeId" = "csoType"."id" LEFT outer JOIN (SELECT "us"."id" AS "assistId", "us"."email" as "email", "upd"."firstName'+lang+'" as "firstName", "upd"."lastName'+lang+'" as "lastName", "upd"."occupation'+lang+'" as "occupation", "upd"."tel" as "tel", "upd"."mobile" as "mobile",  "p"."id" AS "partnerId" FROM "users" as "us" LEFT JOIN "users_has_roles" as "uhr" ON "us"."id" = "uhr"."userId" LEFT JOIN "partners" as "p" ON "p"."id" = "us"."partnerId" LEFT OUTER JOIN "users_personal_data" as "upd" ON "us"."id"="upd"."userId"  WHERE "uhr"."roleId" = \'' +Role.partnerAssistId+ '\' AND "us"."partnerId" = ' +partnerId+ ' LIMIT 1) as "assist" ON "assist"."partnerId" = "partner"."id" LEFT outer JOIN (SELECT "us"."id" AS "authorisedId", "us"."email" as "email", "upd"."firstName'+lang+'" as "firstName", "upd"."lastName'+lang+'" as "lastName", "upd"."occupation'+lang+'" as "occupation", "upd"."tel" as "tel", "upd"."mobile" as "mobile", "p"."id" AS "partnerId" FROM "users" as "us" LEFT JOIN "users_has_roles" as "uhr" ON "us"."id" = "uhr"."userId" LEFT JOIN "partners" as "p" ON "p"."id" = "us"."partnerId" LEFT OUTER JOIN "users_personal_data" as "upd" ON "us"."id"="upd"."userId" WHERE "uhr"."roleId" = \'' + Role.partnerAuthorisedId + '\' AND "us"."partnerId" = ' +partnerId+ ' LIMIT 1) as "authorised" ON "authorised"."partnerId" = "partner"."id" WHERE "partner"."id" = ' +partnerId+ ' LIMIT 1', {
            type: QueryTypes.SELECT,
            nest: true,
            plain: true
        });
