...
class RequestBadStatus extends HttpException {
    /**
    ...
    constructor(status?: number, errorCode?: number, message?: string, devMessage?: string) {
        ...
        const responseMessage: string = message || i18n.t('requestBadStatusError');
        const responseDevMessage: string = devMessage || 'Request status does not allow action';
        super(responseStatus, responseErrorCode, responseMessage, responseDevMessage);
    }
}

export default RequestBadStatus;