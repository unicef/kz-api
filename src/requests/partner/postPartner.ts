...
const postPartner = (req: Request, res: Response, next: NextFunction) => {
    let validationRules: any = {
        bodySchema: Joi.object().options({
            ...
            seoFirstNameEn: Joi.string().max(255).required(),
            seoFirstNameRu: Joi.string().max(255).required(),
            seoLastNameEn: Joi.string().max(255).required(),
            seoLastNameRu: Joi.string().max(255).required(),
